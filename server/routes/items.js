 ...
 * @access Public
 */
router.post("/men/:name", function(req, res) {
    const { username, token, name, size, num } = req.body;
    Items.findOne({ name: name }, async function(err, items) {
        let favoirte = await new ShoppingCar({
            username: username,
...
/**
 * @route post /women/:name
 * @desci get shoppingCar
 * @access Public
 */
router.post("/women/:name", function(req, res) {
    const { username, token, name, size, num } = req.body;
    Items.findOne({ name: name }, async function(err, items) {
        let favoirte = await new ShoppingCar({
            username: username,
...
/**
 * @route post /kids/:name
 * @desci get shoppingCar
 * @access Public
 */
router.post("/kids/:name", function(req, res) {
    const { username, token, name, size, num } = req.body;
    Items.findOne({ name: name }, async function(err, items) {
        let favoirte = await new ShoppingCar({
            username: username,
